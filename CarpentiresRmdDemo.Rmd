---
title: "Carpentires Example Streamflow Analysis"
...
This is an example script developed durng a Carpentires workshop. Within it, we will develope a workflow to:
...
We will load any relevant packes and set the default for our code visibility in the final report. 
...
In this section, we will perform some basic data cleaning operations to get our data ready for future analysis. 
...
We analyzed data from USGS gage `r usgs_id`. During the period of record, the maximum stage was `r max(data_clean$stage_ft)` and the minum stage was `r min(data_clean$stage_ft)`. There were a total of `r sum(data_clean$stage_QAcode == "Gapfill")` missing dates that we gap-filled using linear interpolation. 
